...
const UpdateOrder = () => {
  ...
  const [pendingGetOrderDetail, getOrderDetail] = useHandleAsyncRequest(
    useCallback(async () => {
      const { ok, body, error } = await orderApi.getOrderDetail(id);
      if (ok && body) {
        setOrderDetail(body);
      }
      if (error) {
  ...
  useEffect(() => {
    dispatch(pendingGetOrderDetail ? incrementLoading() : decrementLoading());
  }, [pendingGetOrderDetail, dispatch]);
  ...
  useEffect(() => {
    if (orderDetail) {
      form.setFieldsValue({
        orderDetails: orderDetail.orderDetails.map((record) => ({
